...
const Menu_card = (props) => {
    ...
    const handledispatch=(itr)=>{
      dispatch(addItems(itr));
    }
    return (
    <div className="flex justify-between max-w-5xl border-b-2 p-4 mx-auto gap-8 menu-card">
              ...
              <div>
              <img className=" select-none w-52 h-44 border rounded-xl object-cover menu-img" src={"https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_300,h_300,c_fit/"+imageId} 
              onError={(e) => {
              ...
              >
                <p className=" select-none px-11 text-green-600 rounded-xl border border-solid border-gray-300 shadow-lg py-1 font-bold text-lg text-center relative bottom-3 bg-white" >ADD</p>
              </div>
              </div>
  </div>
 );
